 ...
 *       CLASS: Node
 * 
 *       Definition: the root of a local subtree inside a directed tree Role:
 *       define the ID/LP relation to other nodes in the tree Responsibility: -
 *       access its own local properties - access its own structural properties
 *       - navigate inside the dominated subtree - access dominated span of
 *       terminals - distinguish node types (root, internal, leaf) - identify
 *       projection nodes and handle them ---> flaten ---> binarize ---> HD
 *       processes ---> etc.
 * 
 *       TODO: - navigate inside subtrees - remove node - remove subtree -
 *       remove and re-attache daughters - flatten projections
 * 
 */
...
public class Node {
	...
	public String toStringSubtree() {
		...
		if (!this.isLeaf()) {
			...
			boolean bFirst = true;
			for (Iterator<Node> iterator = getDaughters().iterator(); iterator.hasNext();) {
	...
	public List<Terminal> getTerminals() {
		...
		if (this instanceof Terminal) {
			...
			for (Node daughter : getDaughters()) {
				lst.addAll(daughter.getTerminals());
			}
			return lst;
		}
	}
