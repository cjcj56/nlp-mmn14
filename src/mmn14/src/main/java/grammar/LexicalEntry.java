 ...
 *         CLASS: lexical entry
 * 
 *         Definition: The set of analyses for a terminal of a language as
 *         listed in its lexicon Role: Map the lexical entry onto a set of
 *         possible grammatical contexts (symbols) that can generate it
 *         Responsibility: Maintain consistency with the set(s) of grammar
 *         terminal and pre-terminal categories
 * 
 */
public class LexicalEntry {
	...
	public Rule getRule(Rule rule) {
		if (getLexicalRules().contains(rule)) {
			for (Rule rule2 : getLexicalRules()) {
